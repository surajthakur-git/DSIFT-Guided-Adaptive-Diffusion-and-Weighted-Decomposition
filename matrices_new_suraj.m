function metrics = matrices_new_suraj(xrcw, a,b)
...
%% API
F_uint8_d = double(F_uint8);  % Ensure it's double for precision
API = mean(F_uint8_d(:));
fprintf('API = %.6f\n', API);


%%% Standard Deviation (SD).
%%% Standard Deviation (SD) using uint8 image
F_uint8_d = double(F_uint8);
SD = std(F_uint8_d(:));
fprintf('SD = %.6f\n', SD);

%%% Average Gradient (AG) using uint8 image
tmp1 = [F_uint8_d(2:p,:); F_uint8_d(1,:)];
tmp2 = [F_uint8_d(:,2:q), F_uint8_d(:,1)];
tmp = sqrt((F_uint8_d - tmp1).^2 + (F_uint8_d - tmp2).^2);
AG = sum(tmp(:)) / (p*q);
fprintf('AG = %.6f\n', AG);
...
F_double = im2double(xrcw);
A_double = im2double(a);

% ERGAS: reference = first source image (a)
if ~isequal(size(F_double,1:2), size(A_double,1:2))
    A_resized = imresize(A_double, size(F_double,1:2));
else
    A_resized = A_double;
end

ratio = size(A_double,1) / size(F_double,1);  % low-res / high-res
rmse2 = mean((F_double - A_resized).^2, [1 2]);
mu2   = mean(A_resized.^2, [1 2]) + eps;
ergas_val = 100 / ratio * sqrt(mean(rmse2 ./ mu2));
